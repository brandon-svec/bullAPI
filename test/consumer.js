...
describe('Consumer Handler', function () {
  describe('Initialization', function () {
    ...
    it('Read Dir Fails', function (done) {
      const stub = sinon.stub(fs, 'readdir').callsFake(function (dirpath, cb) {
    ...
    it('Queue fails to Init - Callback', function (done) {
      const stub = sinon.stub(defaultQueue, 'Init').callsFake(function (cb) {
        return setImmediate(cb, new Error('Something Broke'));
      });
    ...
    it('Queue fails to Init - throw', function (done) {
      const stub = sinon.stub(defaultQueue, 'Init').callsFake(function () {
  ...
  describe('Get Schemas', function () {
    ...
    it('Not Existent Consumer', function (done) {
      const schema = consumer.GetSchema('doesNotExist');
  ...
  describe('Exists', function () {
    it('Hit', function (done) {
      const exists = consumer.Exists('default');
      assert.isTrue(exists);
      done();
    });

    it('Miss', function (done) {
      const exists = consumer.Exists('doesNotExist');
  ...
  describe('Process Work Handler', function () {
    ...
    before(function () {
      sinon.stub(queueFactory, 'GetQueue').callsFake(function () {
        return {
          process: function () {
    ...
    it('Processes Successfully', function (done) {
      ...
      const stub = sinon.stub(defaultQueue, 'ProcessWork').callsFake(function (jobObject, cb) {
