		...
		describe('Repeating', function () {
			...
			describe('Get Jobs', function () {
				it('Gets All Jobs', function (done) {
					let jobs = [{ id: 'test1', data: { a: 'apple' } }];

					let stub = sinon.stub(scheduler, 'GetRepeatingJobs').callsFake(function (queue, cb) {
						assert.equal(queue, 'default');
			...
			describe('Delete Job', function () {
				it('Success', function (done) {
					let stub = sinon.stub(scheduler, 'DeleteRepeatingJob').callsFake(function (queue, name, cb) {
						assert.equal(queue, 'default');
						assert.equal(name, 'test1');
						return setImmediate(cb);
					});
