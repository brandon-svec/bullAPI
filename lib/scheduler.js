  ...
  async.waterfall([
    (cb) => {
      GetRepeatingJob(queue, name, function (err, job) {
        ...
        if (job) {
          const eErr = new EnhancedError('Job Already Exists', true);
          return cb(eErr);
        }
...
function GetRepeatingJob (queueName, jobName, cb) {
  GetRepeatingJobs(queueName, function (err, jobList) {
    if (err) {
      const eErr = new EnhancedError(err.message, false);
      return cb(eErr);
    }
...
function GetRepeatingJobs (queueName, cb) {
  if (!queueFactory.Exists(queueName)) {
    const eErr = new EnhancedError('Queue Not Found', true);
    return cb(eErr);
  }
  ...
  async.waterfall([
    (cb) => {
      GetRepeatingJob(queueName, jobName, function (err, job) {
        ...
        if (!job) {
          const eErr = new EnhancedError('Job Not Found', true);
          return cb(eErr);
        }
