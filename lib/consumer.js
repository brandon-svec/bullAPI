    ...
    try {
      consumer.ProcessWork(jobObj, function (err) {
        if (err) {
          ...
          log.warn({ Error: err.message, Envelope: jobObj.data, Queue: name, AttemptsMade: jobObj.attemptsMade, DelaySec: (jobObj.processedOn - jobObj.timestamp) / 1000 }, 'Consumer Work Failed. Retrying...');
          return cb(err);
        }

        log.info({ Queue: name, DelaySec: (jobObj.processedOn - jobObj.timestamp) / 1000 }, 'Consumer Work Processed');
        return cb();
      });
    } catch (err) {
      log.error({ Error: err.message, Envelope: jobObj.data, Queue: name, DelaySec: (jobObj.processedOn - jobObj.timestamp) / 1000 }, 'Consumer Work Unrecoverable Error');
