...
function initializeConsumer (file, cb) {
	...
	try {
		...
		consumer.Init(function (err) {
			...
			queue.process(GenerateWorkHandler(name, consumer));
...
function GenerateWorkHandler (name, consumer) {
	...
	return function (jobObj, cb) {
		try {
          ...
          log.warn({ Error: err.message, Envelope: jobObj.data, Queue: name, AttemptsMade: jobObj.attemptsMade, DelaySec: (jobObj.processedOn - jobObj.timestamp - jobObj.opts.delay) / 1000 }, 'Consumer Work Failed. Retrying...');
					return cb(err);
				}

				log.info({ Queue: name, DelaySec: (jobObj.processedOn - jobObj.timestamp - jobObj.opts.delay) / 1000 }, 'Consumer Work Processed');
				return cb();
			});
		} catch (err) {
      log.error({ Error: err.message, Envelope: jobObj.data, Queue: name, DelaySec: (jobObj.processedOn - jobObj.timestamp - jobObj.opts.delay) / 1000 }, 'Consumer Work Unrecoverable Error');
